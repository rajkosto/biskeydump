...
void derive_nx_keydata(u32 target_firmware) {
    ...
    decrypt_data_into_keyslot(0xF, KEYSLOT_SWITCH_SBK, work_buffer, 0x10); //decrypt that with SBK into slot 0xF
    se_aes_ecb_decrypt_block(0xD, work_buffer, 0x10, keyblob_seeds[MASTERKEY_REVISION_500_CURRENT], 0x10); //decrypt keyblob_seed into work_buffer using TSEC key
    decrypt_data_into_keyslot(0xD, KEYSLOT_SWITCH_SBK, work_buffer, 0x10); //decrypt that with SBK into slot 0xD, 0xD now has keyblob_key
    
    /* Clear the SBK. */
    clear_aes_keyslot(KEYSLOT_SWITCH_SBK); //dont need SBK anymore
    se_aes_ecb_decrypt_block(0xD, work_buffer, 0x10, keyblob_mac_seed, 0x10); //decrypt keyblob_mac_seed into work_buffer using keyblob_key
