...
int max77620_regulator_get_status(u32 id)
{
	...
	if (reg->type == REGULATOR_SD)
		return i2c_recv_byte(I2C_PWR, 0x3C, MAX77620_REG_STATSD) & reg->status_mask ? 0 : 1;
	return i2c_recv_byte(I2C_PWR, 0x3C, reg->cfg_addr) & 8 ? 1 : 0;
}
...
int max77620_regulator_config_fps(u32 id)
{
	...
	i2c_send_byte(I2C_PWR, 0x3C, reg->fps_addr, (reg->fps_src << 6) | (reg->pu_period << 3) | (reg->pd_period));
...
int max77620_regulator_set_voltage(u32 id, u32 mv)
{
	...
	u8 val = i2c_recv_byte(I2C_PWR, 0x3C, reg->volt_addr);
	val = (val & ~reg->volt_mask) | (mult & reg->volt_mask);
	i2c_send_byte(I2C_PWR, 0x3C, reg->volt_addr, val);
	msleep(1);
...
int max77620_regulator_enable(u32 id, int enable)
{
	...
	u8 val = i2c_recv_byte(I2C_PWR, 0x3C, addr);
	if (enable)
		val = (val & ~reg->enable_mask) | ((3 << reg->enable_shift) & reg->enable_mask);
	else
		val &= ~reg->enable_mask;
	i2c_send_byte(I2C_PWR, 0x3C, addr, val);
	msleep(1);
...
void max77620_config_default()
{
	...
	{
		i2c_recv_byte(I2C_PWR, 0x3C, MAX77620_REG_CID4);
	...
	i2c_send_byte(I2C_PWR, 0x3C, MAX77620_REG_SD_CFG2, 4);
}
